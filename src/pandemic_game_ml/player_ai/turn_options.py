...
from .turn_intent import ActionCost, MovementPossibilities
...
class TurnOptions:
        ...
        while queue:
            ...
            for neighbor in current_location.connections:
                if neighbor not in visited:
                    ...
                    movementPossibilities.add_possibility(
                        neighbor.city, ActionCost(distance + 1)
                    )
                    queue.append((neighbor, distance + 1))
        return movementPossibilities

    @staticmethod
    def locations_can_direct_flight_to(
        player_cards: list[PlayerDeckCard],
    ) -> MovementPossibilities:
        movementPossibilities = MovementPossibilities()
        for card in player_cards:
            if (
                card == PlayerDeckCard.EPIDEMIC
            ):  # This is accidental complexity and can be improved with better types
    ...
    @staticmethod
    def locations_can_charter_flight_to(
        player_card: PlayerDeckCard, location: Location
    ) -> MovementPossibilities:
        movementPossibilities = MovementPossibilities()
        ...
        if player_card.value == location.city:
            movementPossibilities.add_possibility(
                player_card.value, ActionCost(1, player_card.value)
            )
        return movementPossibilities
