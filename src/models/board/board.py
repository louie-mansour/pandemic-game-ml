...
class Board:
    def __init__(self):
        self.location_graph: LocationGraph = LocationGraph.static_factory()
    ...
    def add_disease_cubes_to_location(self, city: City, qty: int, colour: Colour) -> Outbreak:
    ...
    def _add_cubes_from_pool(self, colour: Colour, qty: int, location: Location) -> Outbreak:
        if colour == Colour.RED:
            self.disease_cube_pool.remove_cubes(Colour.RED, qty)
            return location.add_red_cubes(qty)
        elif colour == Colour.BLUE:
            self.disease_cube_pool.remove_cubes(Colour.BLUE, qty)
            return location.add_blue_cubes(qty)
        elif colour == Colour.YELLOW:
            self.disease_cube_pool.remove_cubes(Colour.YELLOW, qty)
            return location.add_yellow_cubes(qty)
        elif colour == Colour.BLACK:
            self.disease_cube_pool.remove_cubes(Colour.BLACK, qty)
